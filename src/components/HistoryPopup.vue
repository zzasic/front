...
<template>
  ...
  <div
    ...
    <div
      ...
      >
      <div class="cust-result" :style="{ width: '392px' }">
        <h3 style="padding-bottom: 10px;">기본정보</h3>
        <div style="height: 100%;">
          <vuescroll>
            <v-row>
              <v-col cols="3" class="col-head">
      ...
      <div class="chat-wrap">
        <vuescroll :style="{ padding: '0px 20px 0px '}"
          ref='vs'>
          <div class="chat-discussion">
            <div v-for="(hist, idx) in callHistories" :key="idx" class="chat-message" :class="hist.moudule === 'TTS' ? 'left' : 'right'">
      ...
      <div v-if="sttHist && sttHist.historyId" class="mock-result" :style="{ width: mockWidth }">
        <h3 style="padding-bottom: 10px;">상세내역</h3>
        <div style="height: 100%;">
          <vuescroll>
            <v-row v-for="hist in sttHistories" :key="hist.historyId">
              <v-col cols="2" class="col-head">
                <div>
                  <i class="icon icon-lg-comment-fill"></i>
                  <span>{{ hist.moudule === 'CHATBOT_IN' ? 'FILTER' : hist.moudule === 'CHATBOT_OUT' ? 'CHATBOT' : hist.moudule }}</span>
                </div>
              </v-col>
              <v-col class="pl-3 pr-0">
                <v-row v-if="hist.moudule === 'CHATBOT_OUT' && hist.scenarioNm !== null && hist.scenarioNm !== ''">
                  <v-col class="pa-0">
                    시나리오 : {{ hist.scenarioNm }}
                  </v-col>
                </v-row>
                <v-row>
                  <v-col class="pl-0 pr-0">
                    <ul class="chips">
                      <li v-html="replaceHtml(hist.contents)"></li>
                    </ul>
                  </v-col>
                </v-row>
              </v-col>
