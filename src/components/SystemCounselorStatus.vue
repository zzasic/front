...
export default {
  ...
  computed: {
    isCounselorTask: function () {
      return !this.counselor || (!this.counselor.systemId || !this.counselor.tenantId || !this.counselor.branchCd || !this.counselor.deviceNo) ? '' : !this.counselor.taskNm ? '' : 'task'
    },
    ...
    statusClass: function () {
      ...
      return !this.counselor || (!this.counselor.systemId || !this.counselor.tenantId || !this.counselor.branchCd || !this.counselor.deviceNo) ? 'fail' : this.counselor.connectYn === 'N' ? 'fail' : this.counselor.connectYn === 'C' ? 'connecting' : this.counselor.callYn === 'Y' ? (this.counselor.abnormalCount > 0 ? 'abnormal' : 'on') : 'wating'
    },
    statusIconClass: function () {
      return !this.counselor || (!this.counselor.systemId || !this.counselor.tenantId || !this.counselor.branchCd || !this.counselor.deviceNo) ? 'icon-lg-tabler_phone-off' : this.counselor.connectYn === 'N' ? 'icon-lg-tabler_phone-off' : this.counselor.connectYn === 'C' ? 'icon-lg-tabler_phone-calling' : this.counselor.callYn === 'Y' ? 'icon-lg-teenyicons_headset-outline' : 'icon-lg-tabler_phone-call'
    }
  },
  data () {
    return {
