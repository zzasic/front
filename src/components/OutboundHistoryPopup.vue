...
<template>
  ...
  <div
    ...
    <div
      ...
      <div v-else class="scenario-result" :style="{ width: mockWidth }">
        <h3 class="col-title">시나리오 진행단계</h3>
        <div class="col-wrap">
          <vuescroll>
            <v-row class="scenario-result__item" v-for="(nodeInfo, idx) in scenarioNodes" :key="nodeInfo.historyId" :class="{ 'first': idx === 0 }">
              <v-col class="scenario-result__left">
                <span class="scenario-result__point">{{ idx + 1 }}</span>
              </v-col>
              <v-col class="scenario-result__right">
                <div class="scenario-result__history">
                  <p>
                    {{ nodeInfo.scenarioNm }}
                    <span class="scenario-result__arrow">></span>
                    <strong>{{ nodeInfo.nodeNm }}</strong>
                  </p>
                </div>
              </v-col>
            </v-row>
    ...
    <div
      ...
      <v-btn
        v-if="sttHist && sttHist.historyId"
        class="btn-reset"
        :ripple="false"
        @click="closeSttHistPopup()">
        상세내역 닫기
      </v-btn>
      <v-btn
        class="btn-reset"
...
export default {
  ...
  computed: {
    ...
    scenarioNodes: function () {
      return this.callAllHistories.filter((ih) => {
        return ih.moudule === 'CHATBOT_OUT' && ih.scenarioNm !== null && ih.scenarioNm !== '' && ih.nodeNm !== null && ih.nodeHm !== ''
      })
    },
  ...
  methods: {
    ...
    centerWindow () {
      if (!this.chat.systempopup) {
        ...
        // console.log((ctx, pageWidth / 2) - (elemWidth / 2) + diff + 'px', (pageHeight / 2) - (elemHeight / 2) + diff + 'px')
