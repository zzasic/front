    ...
    <vuescroll>
        ...
        <v-row no-gutters>
          <v-col cols="2">
            <v-select
              ...
              :placeholder="searchForm.tenant ? undefined : $t('label.choice')"
              clearable
            ></v-select>
          </v-col>
          <v-col cols="2">
          ...
          <v-col cols="2">
            <v-select
              ...
              label="시나리오"
              :placeholder="searchForm.scenario ? undefined : $t('label.choice')"
              clearable
            ></v-select>
          </v-col>
          <v-col cols="2">
      ...
      <v-card class="data-grid-wrap default clickable">
        ...
        >
          <template v-slot:item="props">
            <tr @click="detailRow(props.item), selected = props.item.scenarioId">
              <td class="text-center">{{ props.item.scenarioNm }}</td>
              <td class="text-left">{{ props.item.lastNodeId }}</td>
              <td class="text-left">{{ props.item.nodeNm }} {{ 1 === props.item.isTransfer ? ' - 호전환' : '' }}</td>
              <td class="text-center">{{ props.item.nodeCount}}</td>
              <td class="text-center">{{ props.item.nodeRate}}</td>
      ...
      <div class="pagination-group">
        ...
        <v-pagination
          ...
          @input="getOutboundProcessingList"
          :total-visible="10"
        ></v-pagination>
      </div>
...
export default {
  ...
  mounted () {
    this.initOutboundProcessingView()
  },
  data () {
    return {
      processingViewHeaders: [
        { text: '시나리오명', value: 'scenarioNm', align: 'center', width: '250px' },
        { text: '노드ID', value: 'lastNodeId', align: 'center', width: '150px' },
        { text: '노드명', value: 'nodeNm', align: 'center' },
        { text: '건수', value: 'nodeCount', align: 'center', width: '100px' },
        { text: '비율', value: 'nodeRate', align: 'center', width: '100px' },
        { text: '상세내역', value: 'buttonhere', align: 'center', class: 'text-center', width: '80px', sortable: false }
      ],
      processingList: [],
      pagination: {
        page: 1, // 현재페이지
  ...
  computed: {
    ...
    optionSync: {
      ...
      set: function (options) {
        ...
        if (doit) {
          this.getOutboundProcessingList()
        }
      }
    }
  },
  ...
  methods: {
    async initOutboundProcessingView () {
      await this.getOutboundProcessingSearchCondition()
      this.getOutboundProcessingList()
    },
    getOutboundProcessingSearchCondition: function () {
      getOutboundProcessingSearchCondition().then(
        response => {
    ...
    getOutboundProcessingList: function () {
      /* datepicker open */
      ...
      getOutboundProcessingList(searchCondition).then(
        response => {
          this.processingList = response.data.result.processingList ? response.data.result.processingList : []
          // paging setting
          this.pagination.totalRows = response.data.result.processingListCount
    ...
    excelDown: function () {
      ...
      reqExcelDownOutboundProcessingList(searchCondition).then(response => {
        const filename = this.$moment().format('YYYY-MM-DD') + '_O/B_단계별_현황.xlsx'
