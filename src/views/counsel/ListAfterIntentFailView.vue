  ...
  <div class="process">
    <vuescroll>
      ...
      <v-container class="search-group" no-gutters fluid>
        <v-row no-gutters>
          ...
          <v-col cols="2">
            <v-text-field
              class="default search"
              v-model="searchForm.sourceContents"
              label="고객발화"
              placeholder=" "
              hide-details
              clearable
      ...
      <v-card class="data-grid-wrap default clickable">
        <v-data-table
          :headers="intentFailViewHeaders"
          :items="intentFailList"
        ...
        >
          <template v-slot:item="props">
            <tr @click="detailRow(props.item), selected = props.item.callId">
              ...
              <td class="text-center">{{ props.item.lastUpdTime ? $moment(props.item.lastUpdTime, 'YYYY-MM-DDTHH:mm:ss.SSSZ').zone('+09:00').format('MM-DD HH:mm:ss') : '' }}</td>
...
export default {
  ...
  mounted () {
    this.initAfterIntentFailView()
  },
  data () {
    return {
      intentFailViewHeaders: [
        { text: '테넌트', value: 'tenantNm', align: 'center', width: '120px' },
        { text: '통화일시', value: 'startDt', align: 'center', width: '120px' },
        { text: '시나리오', value: 'scenarioNm', align: 'left', class: 'text-center', width: '250px' },
        { text: '처리일시', value: 'lastUpdTime', align: 'center', width: '120px' },
  ...
  computed: {
    ...
    cptdItemsTransferList () {
      ...
      transferList.push(...this.transfers)
      return transferList
    },
    dateRangeText: {
      get: function () {
  ...
  methods: {
    async initAfterIntentFailView () {
      await this.getAfterIntentFailSearchCondition()
      this.getAfterIntentFailList()
    },
    getAfterIntentFailSearchCondition: function () {
      getAfterIntentFailSearchCondition().then(
        response => {
    ...
    getAfterIntentFailList: function () {
      /* datepicker open */
      ...
      const searchCondition = {
        ...
        sourceContents: this.searchForm.sourceContents,
      ...
      getAfterIntentFailList(searchCondition).then(
        response => {
          this.intentFailList = response.data.result.intentFailList ? response.data.result.intentFailList : []
          // paging setting
          this.pagination.totalRows = response.data.result.intentFailListCount
    ...
    excelDown: function () {
      ...
      reqExcelDownAfterIntentFail(searchCondition).then(response => {
        const filename = this.$moment().format('YYYY-MM-DD') + '_I/B_의도추론_실패(사후).xlsx'
