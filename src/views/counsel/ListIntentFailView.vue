...
export default {
  ...
  methods: {
    async initIntentFailView () {
      await this.getIntentFailSearchCondition()
      this.getIntentFailList()
    },
    getIntentFailSearchCondition: function () {
      getIntentFailSearchCondition().then(
        response => {
    ...
    getIntentFailList: function () {
      ...
      getIntentFailList(searchCondition).then(
        response => {
    ...
    excelDown: function () {
      ...
      reqExcelDownIntentFail(searchCondition).then(response => {
        const filename = this.$moment().format('YYYY-MM-DD') + '_I/B_의도추론_실패_현황.xlsx'
