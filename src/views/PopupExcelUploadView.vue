  ...
  methods: {
    ...
    saveBtn: function () {
      ...
      if (selected.length > 0) {
        postRequest(`/api/excel/save/${this.code}`, JSON.stringify({
          selected: selected
        })).then(
          (response) => {
            const status = response.data.status
            if (status === 200) {
              ...
              let successCount = 0
              let failCount = 0
              for (let a = 0; a < this.xlsDatas.length; a++) {
                this.xlsDatas[a].selected = []
              }
              for (let i = 0; i < result.length; i++) {
                if (result[i].success === true) {
                  successCount++
                  // 성공 수정 불가 처리
                  for (let j = 0; j < this.xlsDatas.length; j++) {
                    for (let k = 0; k < this.xlsDatas[j].deserts.length; k++) {
                      if (result[i].eid === this.xlsDatas[j].deserts[k].eid) {
                        this.xlsDatas[j].deserts[k].isSelectable = false
                      }
                    }
                  }
                } else {
              ...
              alert('총 건수: ' + this.comma(totalCount) + ' 건\n- 성공건수: ' + this.comma(successCount) + ' 건\n- 실패건수: ' + this.comma(failCount) + ' 건\n처리되었습니다')
