<template>
  <v-card class="dialogTerms">
    <v-card-title>
      <span class="headline">약관 수정</span>
    </v-card-title>
    <v-card-text>
      <vuescroll>
        <v-form
...
<script>
import { updateTermsView, updateTerms, deleteTerms } from '@/api/terms/'
import datepicker from '@/plugins/datepicker'

export default {
  name: 'PopupUpdateTermsView',
  mixins: [datepicker],
  props: {
    termsId: String,
    termsVer: String
  ...
  methods: {
    init: async function () {
      await this.updateTermsView()
    },
    // 입력값 조회
    updateTermsView: function () {
      updateTermsView(this.termsId, this.termsVer).then(
        response => {
          // console.log(response)
          const status = response.data.status
          if (status === 200) {
            this.langs = response.data.result.langList // 언어
            const termsInfo = response.data.result.terms
            this.inputForm = {
              termsVer: termsInfo.termsVer,
              termsLangList: [
                {
                  lang: 'KO',
                  termsTitle: termsInfo.termsTitle,
                  termsContents: termsInfo.termsContents
                },
                ...termsInfo.termsLangList
              ],
              termsDates: [
                termsInfo.termsStartTime,
                termsInfo.termsEndTime
              ],
              necessaryFlag: termsInfo.necessaryFlag === 'Y' ? 'Y' : 'N',
              useYn: termsInfo.useYn === 'Y' ? 'Y' : 'N',
              privacyYn: termsInfo.privacyYn === 'Y' ? 'Y' : 'N'
            }
          }
        },
        error => {
    ...
    updateTerms: function () {
      ...
      const updateForm = {
        ...
        termsStartTime: dateRange && dateRange.length > 0 ? dateRange[0] : '', // 적용시작일
        termsEndTime: dateRange && dateRange.length > 0 ? dateRange.length > 1 ? dateRange[1] : dateRange[0] : '', // 적용종료일
        necessaryFlag: this.inputForm.necessaryFlag === 'Y' ? 'Y' : 'N',
        useYn: this.inputForm.useYn === 'Y' ? 'Y' : 'N',
        privacyYn: this.inputForm.privacyYn === 'Y' ? 'Y' : 'N'
      }
      updateTerms(updateForm).then(
        response => {
          const status = response.data.status
          if (status === 200) {
