...
export default {
  ...
  computed: {
    ...
    getcounselors: function () {
      ...
      return (counselors || []).filter((cs) => cs.callType === 'IN')
    },
    tenantList: function () {
      const its = [
        {
    ...
    scenarioCategoryList: function () {
      return (this.categoryies || []).filter(c => {
        return this.tenants.some(t => c.opt4 === t.value) && c.opt5 === 'IN' && (!this.searchForm.tenantId || this.searchForm.tenantId === c.opt4)
      })
    },
