    ...
    <vuescroll>
      ...
      <v-container class="search-group" no-gutters fluid>
        <v-row no-gutters>
          ...
          <v-col cols="2">
            <v-text-field
              class="default search"
              v-model="searchForm.branchCd"
              label="지점"
              placeholder=" "
              hide-details
              clearable
              v-on:keyup.enter="fnc_getCallBotStatisticsChartList"
            ></v-text-field>
          </v-col>
          <v-col cols="2">
            <v-select
          ...
          <v-col class="text-right">
            <v-btn
              ...
              class="btn-search"
              :ripple="false"
              @click="fnc_getCallBotStatisticsChartList"
      ...
      <div class="pagination-group">
        <v-select
          ...
          @change="fnc_getCallBotStatisticsList"
          solo
        ></v-select>
        <v-pagination
          ...
          @input="fnc_getCallBotStatisticsList"
          :total-visible="10"
        ></v-pagination>
      </div>
      <div class="btn-group align-right">
        <v-btn color="btn-primary" text @click.stop="fnc_reqCallBotStatisticsBatch" v-auth="['SAU']">{{ $t('button.Statistics')}}</v-btn>
...
import {
  getCallBotSearchCondition, // 초기 조회 조건
  getCallBotStatisticsChartList, // 차트 목록 조회
  getCallBotStatisticsList, // 그리드 목록 조회
  reqCallBotStatisticsExcelDown, // 엑셀 다운
...
export default {
  ...
  mounted () {
    ...
    this.fnc_getCallBotStatisticsChartList()
  },
  data () {
    return {
      ...
      headers: [
        ...
        { text: '지점', value: 'branchCd', align: 'center', width: '200px' },
  ...
  computed: {
    ...
    optionSync: {
      ...
      set: function (options) {
        ...
        if (doit) {
          this.fnc_getCallBotStatisticsList()
        }
      }
    },
    yearText: function () {
  ...
  methods: {
    ...
    fnc_getCallBotStatisticsChartList: function () {
      ...
      const searchCondition = {
        ...
        branchCd: this.searchForm.branchCd,
      ...
      getCallBotStatisticsChartList(searchCondition).then(
        response => {
          this.statisticsChartList = response.data.result.callBotStatisticsChartList ? response.data.result.callBotStatisticsChartList : []
        }
      ).finally(() => {
    ...
    fnc_getCallBotStatisticsList: function () {
      const dateRange = []
      if (this.searchForm.timeType === 'B') {
        if (this.pickerYearMenu === true) {
          this.strYear = String(this.$refs.pickerYear.inputYear)
      ...
      this.pagination.loading = true
      getCallBotStatisticsList(searchCondition).then(
        response => {
          this.statisticsList = response.data.result.callBotStatisticsList ? response.data.result.callBotStatisticsList : []
          // paging setting
          this.pagination.totalRows = response.data.result.callBotStatisticsListCount
    ...
    excelDown: function () {
      ...
      reqCallBotStatisticsExcelDown(searchCondition).then(response => {
        const filename = this.$moment().format('YYYY-MM-DD') + '_콜봇_기간별_집계.xlsx'
    ...
    fnc_reqCallBotStatisticsBatch: function () {
      if (!confirm('통계수집을 진행하시겠습니까?')) {
        return
      }
      reqCallBotStatisticsBatch().then(
        response => {
          alert('통계수집을 완료하였습니다.')
          this.fnc_getCallBotStatisticsList()
        },
