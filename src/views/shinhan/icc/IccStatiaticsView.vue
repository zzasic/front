    ...
    <vuescroll>
        ...
        <v-row no-gutters>
          ...
          <v-col cols="2">
            <v-text-field
              ...
              hide-details
              clearable
              v-on:keyup.enter="fnc_getIccStatisticsChartList"
            ></v-text-field>
          </v-col>
          <v-col cols="2">
            <v-select
          ...
          <v-col class="text-right">
            <v-btn
              ...
              class="btn-search"
              :ripple="false"
              @click="fnc_getIccStatisticsChartList"
      ...
      <div class="pagination-group">
        <v-select
          ...
          @change="fnc_getIccStatisticsList"
          solo
        ></v-select>
        <v-pagination
          ...
          @input="fnc_getIccStatisticsList"
          :total-visible="10"
        ></v-pagination>
      </div>
      <div class="btn-group align-right">
        <v-btn color="btn-primary" text @click.stop="fnc_reqIccStatisticsBatch" v-auth="['SAU']">{{ $t('button.Statistics')}}</v-btn>
...
import {
  getIccSearchCondition, // 초기 조회 조건
  getIccStatisticsChartList, // 차트 목록 조회
  getIccStatisticsList, // 그리드 목록 조회
  reqIccStatisticstExcelDown, // 엑셀 다운
  reqIccStatisticsBatch // 통계 테이터 수접
...
export default {
  ...
  mounted () {
    ...
    this.fnc_getIccStatisticsChartList()
  },
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'TA_GW(건)', value: 'taGwCnt', align: 'center', width: '200px' },
        { text: 'TA_TA(건)', value: 'taTaCnt', align: 'center', width: '200px' }
      ],
      statisticsList: [],
  ...
  methods: {
    ...
    fnc_getIccStatisticsChartList: function () {
      ...
      getIccStatisticsChartList(searchCondition).then(
        response => {
          this.statisticsChartList = response.data.result.iccStatisticsChartList ? response.data.result.iccStatisticsChartList : []
        }
      ).finally(() => {
    ...
    fnc_getIccStatisticsList: function () {
      const dateRange = []
      if (this.searchForm.timeType === 'B') {
        if (this.pickerYearMenu === true) {
          this.strYear = String(this.$refs.pickerYear.inputYear)
      ...
      getIccStatisticsList(searchCondition).then(
        response => {
          this.statisticsList = response.data.result.iccStatisticsList ? response.data.result.iccStatisticsList : []
          // paging setting
          this.pagination.totalRows = response.data.result.iccStatisticsListCount
    ...
    fnc_reqIccStatisticsBatch: function () {
      if (!confirm('통계수집을 진행하시겠습니까?')) {
        return
      }
      reqIccStatisticsBatch().then(
        response => {
          alert('통계수집을 완료하였습니다.')
          this.fnc_getIccStatisticsChartList()
        },
