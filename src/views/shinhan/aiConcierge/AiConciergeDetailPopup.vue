        ...
        <div class="col-wrap">
          <vuescroll>
            ...
            <v-row>
              ...
              <v-col class="col-td">
                {{ $moment(aiConciergeCall.startDt).zone('+09:00').format('YYYY-MM-DD HH:mm') }}
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="3" class="col-th">
        ...
        <div class="col-wrap">
          <vuescroll ref='vs'>
            <div class="chat-discussion">
                <div v-for="(hist, idx) in aiConciergeCallHistoryList" :key="idx" class="chat-message" v-show="hist.moudule !== 'CHATBOT_OUT' || onlyStt === 'N'" :class="hist.moudule === 'CHATBOT_OUT' ? 'left' : 'right'">
...
export default {
  ...
  data () {
    return {
      onlyStt: 'Y',
      custInfo: {},
      aiConciergeCallHistoryList: [],
      aiConciergeCallAllHistoryList: [],
      aiConciergeCall: {},
      chatSub: null,
      height: '600px',
      isdown: false,
  ...
  computed: {
    ...
    isScenarios: function () {
      return this.aiConciergeCallAllHistoryList.some((ih) => {
        return ih.moudule === 'CHATBOT_OUT' && ih.scenarioNm !== null && ih.scenarioNm !== ''
      })
    },
    scenarioNms: function () {
      return Object.values(this.aiConciergeCallAllHistoryList.filter((ih) => {
    ...
    sttHistories: function () {
      ...
      if (this.sttHist.historyId) {
        let befMoudule = 'STT'
        let isFirst = true
        for (const ih of this.aiConciergeCallAllHistoryList) {
          // STT
  ...
  methods: {
    ...
    fnc_getAiConciergeHistoryDetailList () {
      getAiConciergeHistoryDetailList(this.chat.callId).then(
        response => {
          const status = response.data.status
          if (status === 200) {
            // 조회결과
            this.aiConciergeCall = response.data.result.aiConciergeCall
            this.aiConciergeCallHistoryList = response.data.result.aiConciergeCallHistoryList
          }
        },
        error => {
          console.error(error)
    ...
    fnc_getAiConciergeAllHistoryDetailList () {
      getAiConciergeAllHistoryDetailList(this.chat.callId).then(
        response => {
          const status = response.data.status
          if (status === 200) {
            // 조회결과
            this.aiConciergeCallAllHistoryList = response.data.result.aiConciergeCallAllHistoryList
          }
        },
        error => {
          console.error(error)
  ...
  mounted () {
    ...
    this.dragElement()
    // this.dragElement()
    this.fnc_getAiConciergeHistoryDetailList()
    this.fnc_getAiConciergeAllHistoryDetailList()
  },
  destroyed () {
  }
}
  ...
  .vue-window-modal-body {
    ...
    .chat-wrap {
      ...
      .chat-discussion {
        ...
        .chat-message.left .message .message-content {
