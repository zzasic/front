      ...
      <v-card-text>
        ...
        <div class="pagination-group">
          <v-pagination
            ...
            @input="getInboundStartTransferList"
            :total-visible="10"
          ></v-pagination>
        </div>
...
import { getInboundStartTransferList } from '../../api/statistics'
export default {
  name: 'PopupStartTransferHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  computed: {
    optionSync: {
      ...
      set: function (options) {
        ...
        if (doit) {
          this.getInboundStartTransferList()
        }
      }
    }
  },
  ...
  methods: {
    init: async function () {
      await this.getInboundStartTransferList()
    },
    // 이력 조회
    getInboundStartTransferList: function () {
      // param setting
      ...
      getInboundStartTransferList(searchCondition).then(
        response => {
          // 조회결과
