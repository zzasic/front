...
import { getInboundTotalList } from '../../api/statistics'
export default {
  name: 'PopupTotalHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getInboundTotalList()
    },
    // 이력 조회
    getInboundTotalList: function () {
      // param setting
      ...
      getInboundTotalList(searchCondition).then(
        response => {
          // 조회결과
