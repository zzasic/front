      ...
      <v-card-text>
        ...
        <div class="pagination-group">
          <v-pagination
            ...
            @input="getScenarioTransferHistoryViewList"
            :total-visible="10"
          ></v-pagination>
        </div>
...
import { getScenarioTransferHistoryViewList } from '../../api/statistics'
export default {
  name: 'PopupScenarioTransferHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  computed: {
    optionSync: {
      ...
      set: function (options) {
        ...
        if (doit) {
          this.getScenarioTransferHistoryViewList()
        }
      }
    }
  },
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: '시나리오', value: 'scenarioNm', align: 'center', width: '250px' },
  ...
  methods: {
    init: async function () {
      await this.getScenarioTransferHistoryViewList()
    },
    // 이력 조회
    getScenarioTransferHistoryViewList: function () {
      // param setting
      ...
      getScenarioTransferHistoryViewList(searchCondition).then(
        response => {
          // 조회결과
