        ...
        >
          <template v-slot:header="{}">
            <thead class="v-data-table-header">
              <tr>
                ...
                <th colspan="2" aria-label="의도추론 실패: Not sorted. Activate to sort ascending." class="text-center" style="width: 200px; min-width: 200px;"><span>의도추론 실패</span></th>
          ...
          <template v-slot:item="props">
            <tr>
              ...
              <td class="text-right"><v-btn @click="startTransferDetailRow(props.item)" text class="default">{{ comma(props.item.startTransferCount) }}</v-btn></td>
...
export default {
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: '호전환', value: 'startTransferCount', align: 'center', width: '200px' },
        { text: '중단', value: 'failCount', align: 'center', width: '200px' },
        { text: '호전환', value: 'transferCount', align: 'center', width: '200px' },
        { text: '정상', value: 'successCount', align: 'center', width: '200px' },
  ...
  watch: {
    pickerYearMenu: function (newVal, oldVal) {
      // console.log('pickerYearMenu', newVal, this.$refs.pickerYear)
  ...
  methods: {
    getSearchCondition: function () {
      getSearchCondition().then(
        response => {
          ...
          // console.log(this.searchForm.itemsTimeTypeList)
        }
      )
    },
    setYear: function () {
    ...
    getStatisticsChartList: function () {
      ...
      getStatisticsChartList(searchCondition).then(
        response => {
          // console.log('차트 정보 : ', response.data.result.statisticsList)
    ...
    getStatisticsList: function () {
      ...
      this.pagination.loading = true
      getStatisticsList(searchCondition).then(
        response => {
    ...
    excelDown: function () {
      ...
      reqExcelDownInbound(searchCondition).then(response => {
        const filename = this.$moment().format('YYYY-MM-DD') + '_I/B_상담_현황.xlsx'
