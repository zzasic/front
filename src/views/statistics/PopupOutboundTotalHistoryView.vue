...
import { getOutboundTotalList } from '../../api/statistics'
export default {
  name: 'PopupOutboundTotalHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getOutboundTotalList()
    },
    // 이력 조회
    getOutboundTotalList: function () {
      // param setting
      ...
      getOutboundTotalList(searchCondition).then(
        response => {
          // 조회결과
