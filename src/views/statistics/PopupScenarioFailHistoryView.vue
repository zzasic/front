...
import { getScenarioFailList } from '../../api/statistics'
export default {
  name: 'PopupScenarioFailHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getScenarioFailList()
    },
    // 이력 조회
    getScenarioFailList: function () {
      // param setting
      ...
      getScenarioFailList(searchCondition).then(
        response => {
          // 조회결과
