...
import { getScenarioTotalList } from '../../api/statistics'
export default {
  name: 'PopupScenarioTotalHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getScenarioTotalList()
    },
    // 이력 조회
    getScenarioTotalList: function () {
      // param setting
      ...
      getScenarioTotalList(searchCondition).then(
        response => {
          // 조회결과
