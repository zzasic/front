      ...
      <v-card-text>
        ...
        <div class="pagination-group">
          <v-pagination
            ...
            @input="getOutboundStartFailList"
            :total-visible="10"
          ></v-pagination>
        </div>
...
import { getOutboundStartFailList } from '../../api/statistics'
export default {
  name: 'PopupOutboundStartFailHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getOutboundStartFailList()
    },
    // 이력 조회
    getOutboundStartFailList: function () {
      // param setting
      ...
      getOutboundStartFailList(searchCondition).then(
        response => {
          // 조회결과
