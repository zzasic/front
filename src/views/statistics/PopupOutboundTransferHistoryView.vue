      ...
      <v-card-text>
        ...
        <div class="pagination-group">
          <v-pagination
            ...
            @input="getOutboundTransferHistoryViewList"
            :total-visible="10"
          ></v-pagination>
        </div>
...
import { getOutboundTransferHistoryViewList } from '../../api/statistics'
export default {
  name: 'PopupOutboundTransferHistoryView',
  props: {
    popup: Object
  ...
  computed: {
    optionSync: {
      ...
      set: function (options) {
        ...
        if (doit) {
          this.getOutboundTransferHistoryViewList()
        }
      }
    }
  },
  ...
  methods: {
    init: async function () {
      await this.getOutboundTransferHistoryViewList()
    },
    // 이력 조회
    getOutboundTransferHistoryViewList: function () {
      // param setting
      ...
      getOutboundTransferHistoryViewList(searchCondition).then(
        response => {
          // 조회결과
