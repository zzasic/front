...
import { getInboundSuccessList } from '../../api/statistics'
export default {
  name: 'PopupSuccessHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getInboundSuccessList()
    },
    // 이력 조회
    getInboundSuccessList: function () {
      // param setting
      ...
      getInboundSuccessList(searchCondition).then(
        response => {
          // 조회결과
