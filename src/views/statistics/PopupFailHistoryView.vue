...
import { getInboundFailList } from '../../api/statistics'
export default {
  name: 'PopupFailHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  methods: {
    init: async function () {
      await this.getInboundFailList()
    },
    // 이력 조회
    getInboundFailList: function () {
      // param setting
      ...
      getInboundFailList(searchCondition).then(
        response => {
          // 조회결과
