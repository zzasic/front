...
import { getOutboundFailList } from '../../api/statistics'
export default {
  name: 'PopupOutboundFailHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: '시나리오', value: 'scenarioNm', align: 'left', class: 'text-center' },
  ...
  methods: {
    init: async function () {
      await this.getOutboundFailList()
    },
    // 이력 조회
    getOutboundFailList: function () {
      // param setting
      ...
      getOutboundFailList(searchCondition).then(
        response => {
          // 조회결과
