            ...
            >
              <template v-slot:item="props">
                <tr @click="detailRow(props.item), selected = props.item.meteringId">
                  ...
                  <td class="text-center">{{ props.item.ttsCount }}</td>
                </tr>
              </template>
            </v-data-table>
          </vuescroll>
...
<script>
import { getTtsHistoryList } from '../../api/usage'
export default {
  name: 'PopupTtsHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  data () {
    return {
      // 페이지 타이틀과 디스크립션
      pageTitle: 'TTS 일별 서비스 사용량',
      pageDescription: 'TTS 일별 서비스 사용량',
      // 테이블  데이터
      headers: [
        { text: '사용일', value: 'meteringDate', align: 'center', class: 'text-center', width: '120px' },
        { text: '사용량', value: 'ttsCount', align: 'center', class: 'text-center', width: '250px' },
  ...
  methods: {
    init: async function () {
      await this.getTtsHistoryList()
    },
    // 이력 조회
    getTtsHistoryList: function () {
      // param setting
      ...
      getTtsHistoryList(searchCondition).then(
        response => {
          // 조회결과
