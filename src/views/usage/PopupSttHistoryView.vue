            ...
            >
              <template v-slot:item="props">
                <tr @click="detailRow(props.item), selected = props.item.meteringId">
                  ...
                  <td class="text-center">{{ props.item.sttCount }}</td>
                </tr>
              </template>
            </v-data-table>
          </vuescroll>
...
<script>
import { getSttHistoryList } from '../../api/usage'
export default {
  name: 'PopupSttHistoryView',
  props: {
    popup: Object
  },
  components: {
  ...
  data () {
    return {
      // 페이지 타이틀과 디스크립션
      pageTitle: 'STT 일별 서비스 사용량',
      pageDescription: 'STT 일별 서비스 사용량',
      // 테이블  데이터
      headers: [
        { text: '사용일', value: 'meteringDate', align: 'center', class: 'text-center', width: '120px' },
        { text: '사용량', value: 'sttCount', align: 'center', class: 'text-center', width: '250px' },
  ...
  methods: {
    init: async function () {
      await this.getSttHistoryList()
    },
    // 이력 조회
    getSttHistoryList: function () {
      // param setting
      ...
      getSttHistoryList(searchCondition).then(
        response => {
          // 조회결과
