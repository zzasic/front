<template>
  <v-card class="dialog-counselor">
    <v-card-title>
    <span class="headline">API 연계 수정</span>
    </v-card-title>
    <v-card-text>
      <v-form
        ref="form"
      ...
      <v-container>
        ...
        <v-row>
          <v-col cols="1">
            <span class="label must">TEST입력값</span>
          </v-col>
          <v-col>
...
export default {
  ...
  methods: {
    ...
    getSelectApiCall: function () {
      selectApiCallForUpdate(this.reqForm).then(
        response => {
          this.inputForm.apiCallId = response.data.result.apiCall.apiCallId
          this.inputForm.systemId = response.data.result.apiCall.systemId
          this.inputForm.tenantId = response.data.result.apiCall.tenantId
          this.inputForm.callType = response.data.result.apiCall.callType
          this.inputForm.callTime = response.data.result.apiCall.callTime
          this.inputForm.apiId = response.data.result.apiCall.apiId
          this.inputForm.realtimeYn = response.data.result.apiCall.realtimeYn
          this.inputForm.useYn = response.data.result.apiCall.useYn
          this.inputForm.script = response.data.result.apiCall.script
        }
      )
    },
    updateApiCall: lodash.debounce(function () {
    ...
    deleteBtn: lodash.debounce(function () {
      ...
      deleteApiCall(this.reqForm).then(
        response => {
          if (response.data.status === 200) {
            this.$emit('refresh-list')
            this.$emit('close-dialog')
