<template>
  <v-card class="dialog-stt">
    <v-card-title>
    <span class="headline">STT 수정</span>
    </v-card-title>
    <v-card-text>
      <v-form
        ref="form"
    ...
    <v-card-actions class="submit-btns-group">
      ...
      <v-btn color="btn-secondary" text @click="deleteStt()">{{ $t('button.delete')}}</v-btn>
...
export default {
  ...
  methods: {
    ...
    getSelectStt: function () {
      selectSttForUpdate(this.reqForm).then(
        response => {
          this.inputForm.sttId = response.data.result.stt.sttId
          this.inputForm.sttNm = response.data.result.stt.sttNm
          this.inputForm.sttUrl = response.data.result.stt.sttUrl
          this.inputForm.sttEngine = response.data.result.stt.sttEngine
          this.inputForm.apiType = response.data.result.stt.apiType
          this.inputForm.useYn = response.data.result.stt.useYn
        }
      )
    },
    ...
    deleteStt: lodash.debounce(function () {
      if (confirm('삭제하시겠습니까?')) {
        deleteStt(this.reqForm).then(
          response => {
            if (response.data.status === 200) {
              this.$emit('refresh-list')
              this.$emit('close-dialog')
