<template>
  <v-card class="dialog-callGroup">
    <v-card-title>
    <span class="headline">Call Group 등록</span>
    </v-card-title>
    <v-card-text>
      <v-form
        ref="form"
...
export default {
  ...
  methods: {
    createCallGroupView: function () {
      createCallGroupView().then(
        response => {
    ...
    saveBtn: function () {
      ...
      if (this.$refs.callGroupNo.validate() === true && !this.errorMessage && this.bCallGroupNoDupCheck === false) {
