<template>
  <v-card class="dialog-counselor">
    <v-card-title>
    <span class="headline">AI 상담원 수정</span>
    </v-card-title>
    <v-card-text>
      <v-form
        ref="form"
...
export default {
  ...
  data () {
    return {
      ...
      callGroups: [],
      inputForm: {
        counselorId: '', // AI 상담원 ID
        tenantId: '', // 테넌트 ID
        extension: '', // AI 상담원 내선번호
        password: '', // 패스워드
        ctiId: '', // CTI ID
        port: '', // UDP 포트
        useYn: 'Y', // 사용여부
      ...
      portRules: [
        value => (!!value && value.length > 0 && value.replace(/\s/g, '').length > 0) || 'UDP 포트를 입력하세요',
  ...
  methods: {
    updateCounselorView: function () {
      updateCounselorView(this.counselorId).then(
        response => {
          ...
          if (status === 200) {
            ...
            Object.assign(this.inputForm, response.data.result.counselor)
          }
        },
        error => {
          console.error(error)
    ...
    extensionDupCheck: function () {
      ...
      this.errorMessages = []
      const input = {
        exceptCounselorId: this.counselorId,
    ...
    saveBtn: function () {
      ...
      if (!this.errorMessage && this.bExtensionDupCheck === false) {
