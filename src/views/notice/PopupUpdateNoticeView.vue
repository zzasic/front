        ...
        >
          <v-container class="noticeView">
            <v-row v-for="(noticeLang, idx) in inputForm.noticeLangList" :key="idx">
              ...
              <v-col cols="1" v-if="langs && langs.length > 1">
                <div class="btn-group align-right">
                  ...
                  <v-btn class="btn-minus" @click="langMinusBtn(idx)" :disabled="!langContentsMinusable || noticeLang.lang === 'KO'">
                ...
                >
                  ...
                  <v-date-picker
                    ...
                    >
                      ...
                      <v-btn text :ripple="false" color="pink" @click="$refs.pickerMenu.save(inputForm.boardDates)">{{ $t('button.confirm')}}</v-btn>
                    </v-date-picker>
                </v-menu>
              </v-col>
            </v-row>
            ...
            <v-row>
              ...
              <v-col cols="5">
                <v-radio-group
                  v-if="inputForm.boardYn == 'N' ? inputForm.popupYn ='N': inputForm.popupYn =inputForm.popupYn"
                  class="default mt-0 pt-0"
...
<script>
import { updateNoticeView, updateNotice, deleteNotice } from '@/api/notice/'
import datepicker from '@/plugins/datepicker'

export default {
  name: 'PopupUpdateNoticeView',
  mixins: [datepicker],
  props: {
    noticeId: String
  },
  ...
  methods: {
    init: async function () {
      await this.updateNoticeView()
    },
    // 입력값 조회
    updateNoticeView: function () {
      updateNoticeView(this.noticeId).then(
        response => {
          const status = response.data.status
          if (status === 200) {
            // this.aiClasses = response.data.result.aiClassList // AI분류
            this.langs = response.data.result.langList // 언어
            const noticeInfo = response.data.result.notice
            this.inputForm = {
              // aiClass: noticeInfo.aiClass,
              noticeLangList: [
                {
                  lang: 'KO',
                ...
                ...noticeInfo.noticeLangList
              ],
              boardDates: [
                noticeInfo.boardStartDate,
                noticeInfo.boardEndDate
              ],
              boardYn: noticeInfo.boardYn === 'Y' ? 'Y' : 'N',
              popupYn: noticeInfo.popupYn === 'Y' ? 'Y' : 'N',
              useYn: noticeInfo.useYn === 'Y' ? 'Y' : 'N'
            }
          }
        },
        error => {
