<template>
  <v-card class="dialog-tts">
    <v-card-title>
    <span class="headline">TTS 수정</span>
    </v-card-title>
    <v-card-text>
      <v-form
        ref="form"
    ...
    <v-card-actions class="submit-btns-group">
      ...
      <v-btn color="btn-secondary" text @click="deleteTts()">{{ $t('button.delete')}}</v-btn>
...
export default {
  ...
  methods: {
    ...
    getSelectTts: function () {
      selectTtsForUpdate(this.reqForm).then(
        response => {
          this.inputForm.ttsId = response.data.result.tts.ttsId
          this.inputForm.ttsNm = response.data.result.tts.ttsNm
          this.inputForm.ttsUrl = response.data.result.tts.ttsUrl
          this.inputForm.ttsEngine = response.data.result.tts.ttsEngine
          this.inputForm.apiType = response.data.result.tts.apiType
          this.inputForm.useYn = response.data.result.tts.useYn
          this.inputForm.basicSpeed = response.data.result.tts.basicSpeed
          this.inputForm.speedInterval = response.data.result.tts.speedInterval
          this.inputForm.maxSpeed = response.data.result.tts.maxSpeed
          this.inputForm.minSpeed = response.data.result.tts.minSpeed
          this.inputForm.basicVolume = response.data.result.tts.basicVolume
          this.inputForm.volumeInterval = response.data.result.tts.volumeInterval
          this.inputForm.maxVolume = response.data.result.tts.maxVolume
          this.inputForm.minVolume = response.data.result.tts.minVolume
        }
      )
    },
    ...
    deleteTts: lodash.debounce(function () {
      if (confirm('삭제하시겠습니까?')) {
        deleteTts(this.reqForm).then(
          response => {
            if (response.data.status === 200) {
              this.$emit('refresh-list')
              this.$emit('close-dialog')
